...
FROM song_playlist SP
         JOIN playlist ON SP.playlist_id = playlist.playlist_id
...
SELECT A.album_id, COUNT(S.song_id) AS počet_skladeb
FROM Album A
         LEFT JOIN Song S ON A.album_id = S.album_id AND S.song_id > 1800 AND S.lyrics IS NOT NULL
GROUP BY A.album_id;


/*6;1;3;Výběr uživatelů, kteří mají nejvíce playlistů*/
SELECT U.user_id, U.username, COUNT(UP.playlist_id) AS počet_playlistů
FROM [User] U
         LEFT JOIN playlist_user UP ON U.user_id = UP.user_id
GROUP BY U.user_id, U.username
HAVING COUNT(UP.playlist_id) = (SELECT MAX(pocet_playlistu)
                                FROM (SELECT user_id, COUNT(playlist_id) AS pocet_playlistu
                                      FROM playlist_user
                                      GROUP BY user_id) AS pocty_playlistu);
