...
bool check(queue<Tag> tagsQueue)
{
    stack<Tag> stack;
    for (int i = 0; i < tagsQueue.size(); i++)
    {
        if (stack.empty() && tagsQueue.front().open)
        {
            stack.push(tagsQueue.front());
            tagsQueue.pop();
            continue;
        }
        if (tagsQueue.front().open)
        {
            stack.push(tagsQueue.front());
            tagsQueue.pop();
        }
        else
        { // tagsQueue.front().open == false
            if (stack.empty())
            {
                error(NotOpened, stack.top(), tagsQueue.front());
                return false;
            }
            if (tagsQueue.front().tt == stack.top().tt)
            {
                tagsQueue.pop();
                stack.pop();
                continue;
            }
            error(NotMatching, stack.top(), tagsQueue.front());
            return false;
        }
    }
    if (stack.empty())
    ...
    error(NotClosed, stack.top(), tagsQueue.front());
    return false;
}
